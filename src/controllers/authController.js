...
exports.register = async(req, res) => {
    try {
        ...
        await db.execute(
            ...
            VALUES (?, ?, ?, ?, 'DOADOR', false, ?, ?, false, 'ativo')`, [nome, email, telefone, senhaHash, codigo, expiraEm]
        );
...
exports.verifyEmail = async(req, res) => {
    try {
        ...
        const token = jwt.sign({ id: user.id, email: user.email, tipo_usuario: user.tipo_usuario },
...
exports.resendCode = async(req, res) => {
    try {
        ...
        await db.execute(
            ...
            WHERE email = ?`, [codigo, expiraEm, email]
        );
    ...
    try {
        ...
        if (user.tipo_usuario === "DOADOR") {
            ...
            if (!user.onboarding_completo) {
                return res.status(403).json({ message: "Finalize o cadastro antes de entrar" });
            }
        }

        if (["SECRETARIO", "ESTOQUE"].includes(user.tipo_usuario)) {
            if (!user.hospital_id) {
                return res.status(403).json({ message: "Usuário não vinculado a um hospital" });
            }
        }
...
exports.completeOnboarding = async(req, res) => {
    ...
    try {
        ...
        if (conhece_tipo_sanguineo === undefined) {
            return res.status(400).json({ message: "O campo 'conhece_tipo_sanguineo' é obrigatório" });
        }
        ...
        if (conhece_tipo_sanguineo) {
            if (!tipo_sanguineo) {
                return res.status(400).json({ message: "Tipo sanguíneo é obrigatório quando o usuário informa que conhece" });
            }
        } else {
            tipo_sanguineo = null;
        }
        ...
        if (user.onboarding_completo) {
            await connection.rollback();
            return res.status(400).json({ message: "Onboarding já foi concluído" });
        }
        ...
        if (existingDonor.length === 0) {
            await connection.execute(
                ...
                VALUES (?, ?, ?, NULL, 0, NOW())`, [userId, tipo_sanguineo, conhece_tipo_sanguineo ? 1 : 0]
            );
        }
        ...
        return res.json({ message: "Onboarding concluído com sucesso" });

    } catch (error) {
        await connection.rollback();
        console.error("COMPLETE ONBOARDING ERROR:", error);
        return res.status(500).json({ message: "Erro ao finalizar onboarding" });
    } finally {
        connection.release();
    }
};
...
exports.salvarPushToken = async(req, res) => {
    try {
        ...
        if (!push_token) {
            return res.status(400).json({ message: "Push token é obrigatório" });
        }
        ...
        return res.status(200).json({ message: "Push token salvo com sucesso" });
