...
async function enviarEmail(to, subject, html) {
    if (isProduction) {
        ...
        await transporter.sendMail({
            from: `"Doate" <${process.env.EMAIL_USER}>`,
            to,
            subject,
            html,
        });
...
async function enviarConviteHospital(email, link) {
    try {
        await enviarEmail(
            email,
            "Convite para acesso ao painel hospitalar - Doate",
            `
...
async function enviarConviteUsuario(email, link, tipo) {
    try {
        ...
        await enviarEmail(
            email,
            "Convite para acesso ao sistema Doate",
            `
            <div style="font-family: Arial, sans-serif; padding: 20px;">
...
async function enviarSolicitacaoHospital(dados) {
    try {
        ...
        await enviarEmail(
            ...
            `Nova solicitação de acesso hospitalar - ${nome}`,
            `
            <div style="font-family: Arial, sans-serif; padding: 20px;">
